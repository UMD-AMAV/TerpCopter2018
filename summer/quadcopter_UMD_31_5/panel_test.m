...
% allow trim alues  between [min_throttle,min_throttle+ trim_lim]
...
%     temp = ((input+handles.stick_lim(1))/handles.trim_lim(1));
    
elseif input<(-handles.stick_lim(1)-handles.trim_lim(1))
    input = -handles.trim_lim(1);
%     temp = ((input+handles.stick_lim(1))/handles.trim_lim(1));
%     input = (-handles.stick_lim(1));
else input = input +handles.stick_lim(1);
end
  % send command
   u_stick_cmd(1:4) = inf;
...
% --- Executes on button press in yaw_rate_radio.
function yaw_rate_radio_Callback(hObject, eventdata, handles)
disp(hObject.Value);
if hObject.Value == 1
    yaw_rate_control();
end
