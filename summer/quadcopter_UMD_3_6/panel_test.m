...
%     input = (-handles.stick_lim(1));
  ...
  % send command
   ...
   textDispVec = [handles.thrustDisplay handles.rollDisplay...
                  handles.pitchDisplay handles.yawDisplay]; 
   send_stick_cmd(u_stick_cmd,trim_scaled,...
   andles.sTrainerBox,handles.stick_lim,handles.pax,textDispVec); 
...
function yaw_control_radio_Callback(hObject, eventdata, handles)
 global yaw_a;
 if hObject.Value == 1
    rosinit('10.0.0.35');%******REPLACE WITH IP ADDRESS OF MASTER NODE***
    yaw_a = rossubscriber('/yawtopic',{@yaw_controller,handles},'BufferSize',1);%*****replace with mavlink topic***
 else
     clear global yaw_a;
     rosshutdown
 end
