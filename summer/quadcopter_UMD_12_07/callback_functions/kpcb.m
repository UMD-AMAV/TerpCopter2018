...
% THIS FUNCTION IS CALLED WHEN USER PRESSES A KEY ON THE KEYBOARD 
% INPUTS:
...
switch( keypressed)
      ...
      function wbmcb(pax,cp1,cp2,sTrainerBox)
          if ~isnan(cp1(1,1)), cp1(1,1)= max(-1,min(1,cp1(1,1)));end
          if ~isnan(cp1(1,2)), cp1(1,2)= max(-1,min(1,cp1(1,2)));end
          %cp1
          %cp2
          if ~isnan(cp2(1,1)), cp2(1,1)= max(-1,min(1,cp2(1,1)));end
          if ~isnan(cp2(1,2)), cp2(1,2)= max(-1,min(1,cp2(1,2)));end
          ...
          % set trim values to NaN here, trim will be set by slider
          % callback
          trim(1:4) = NaN;
