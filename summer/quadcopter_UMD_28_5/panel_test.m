...
set (gcf, 'KeyPressFcn', {@kpcb,handles.sTrainerBox,...
    handles.stick_lim,handles.pax,textDispVec},'units','normalized');
...
textDispVec = [handles.thrustDisplay handles.rollDisplay...
               handles.pitchDisplay handles.yawDisplay]; 
[stickValues, textDispVecUpdated] = send_stick_cmd(u_stick_cmd,trim_scaled,...
              handles.sTrainerBox,handles.stick_lim,handles.pax,textDispVec);


% update display handles 
% handles.thrustDisplay = textDispVecUpdated(1);
% handles.rollDisplay = textDispVecUpdated(2);
% handles.pitchDisplay = textDispVecUpdated(3);
% handles.yawDisplay = textDispVecUpdated(4);
...
% hObject    handle to thrustDisplay (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of thrustDisplay as text
%        str2double(get(hObject,'String')) returns contents of thrustDisplay as a double
input = str2double(get(hObject,'String'));
if isnan(input)|| input>handles.stick_lim(1)||input<-handles.stick_lim(1)
  ...
  % send command
   ...
   [stickValues,~] = send_stick_cmd(u_stick_cmd,trim_scaled,...
              handles.sTrainerBox,handles.stick_lim,handles.pax,textDispVec); 
end
...
% 
% Hints: get(hObject,'String') returns contents of rollDisplay as text
%        str2double(get(hObject,'String')) returns contents of rollDisplay as a double
input = str2double(get(hObject,'String'));
if isnan(input)|| input>handles.stick_lim(2)||input<-handles.stick_lim(2)
  errordlg('Invalid Input','Invalid Input','modal')
  uicontrol(hObject)
  return
else
  % convert input to [-1 1]
    input = input/handles.stick_lim(2);
  % send command
   u_stick_cmd(1:4) = inf;
   u_stick_cmd(2) = input;
   trim_scaled(1:4) = inf;
...
input = str2double(get(hObject,'String'));
if isnan(input)|| input>handles.stick_lim(3)||input<-handles.stick_lim(3)
  errordlg('Invalid Input','Invalid Input','modal')
  uicontrol(hObject)
  return
else
  % convert input to [-1 1]
    input = input/handles.stick_lim(3);
  % send command
   u_stick_cmd(1:4) = inf;
   u_stick_cmd(3) = input;
   trim_scaled(1:4) = inf;
...
input = str2double(get(hObject,'String'));
if isnan(input)|| input>handles.stick_lim(4)||input<-handles.stick_lim(4)
  errordlg('Invalid Input','Invalid Input','modal')
  uicontrol(hObject)
  return
else
  % convert input to [-1 1]
    input = input/handles.stick_lim(4);
  % send command
   u_stick_cmd(1:3) = inf;
   u_stick_cmd(4) = input;
   trim_scaled(1:4) = inf;
