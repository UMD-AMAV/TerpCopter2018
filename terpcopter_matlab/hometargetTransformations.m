...
% CONDITIONS.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [] = hometargetTransformations()
...
%%    
    ...
    % Publishers 
    HomeTargetI_pose = rospublisher('/InertialHomeTargetPose','geometry_msgs/PoseStamped');

    % Messages
    msgHome_IPose = rosmessage(HomeTargetI_pose);
   ...
   while(1)
        msgPose  = receive(pose,10);
        msgHPose = receive(HomevisionPose,10);
      
        msgHPose.Header.FrameId
        ...
        t_C_Tb= [msgHPose.Pose.Position.X; % black target
                msgHPose.Pose.Position.Y;
                msgHPose.Pose.Position.Z;
                ];
...
%            pixhawkYb);
        ...
        msgHome_IPose.Position.X = t_I_T(1); 
        msgHome_IPose.Position.Y = t_I_T(2);
        msgHome_IPose.Header.FrameId = msgHPose.Header.FrameId;

        send(HomeTargetI_pose,msgHome_IPose);
    end
end
